...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect,Ext.form.ComboBox,{
    ...
    onRender:function(ct, position) {
        ...
        this.outerWrapEl.parent().setWidth('auto');
    ...
    initButtonEvents : function() {
        if(this.hideClearButton == false){
            this.buttonClear.addClassOnOver('x-superboxselect-btn-clear-over').on('click', function(e) {
                e.stopEvent();
                if (this.disabled) {
                    return;
                }
    ...
    removeButtonEvents : function() {
        if(this.hideClearButton == false) this.buttonClear.removeAllListeners();
    ...
    onResize : function(w, h, rw, rh) {
        ...
        if(this.wrapEl){
            this._width = w;
            if(this.oneLineOnly == false) this.outerWrapEl.setWidth(w - reduce);
            if (this.renderFieldBtns) {
                reduce += (this.buttonWrap.getWidth() + 20);
                if(this.oneLineOnly == false) this.wrapEl.setWidth(w - reduce);
            }
        }

        this.outerWrapEl.parent().addClass('x-superbox-oneline-wrapper');

        if(this.oneLineOnly == false) Ext.ux.form.SuperBoxSelect.superclass.onResize.call(this, w, h, rw, rh);
        this.autoSize();
    },
    onEnable: function(){
    ...
    manageClearBtn : function() {
        ...
        if (this.items.getCount() === 0) {
            if(this.hideClearButton == false) this.buttonClear.addClass(cls);
            if(this.forceHeight == false) this.outerWrapEl.parent().removeClass('x-superbox-auto-height');
        } else {
            if(this.hideClearButton == false) this.buttonClear.removeClass(cls);
            if(this.forceHeight == false) this.outerWrapEl.parent().addClass('x-superbox-auto-height');
        }
        return this;
    },
    ...
    onDestroy : function() {
        ...
        this.killItems();
        if (this.renderFieldBtns) {
            if(this.hideClearButton == false) Ext.destroy(this.buttonClear);
            Ext.destroy(
                this.buttonExpand,
                this.buttonWrap
            );
